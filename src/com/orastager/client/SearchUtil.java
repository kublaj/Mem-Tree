...
public class SearchUtil {
    ...
    public static void searchMoveNodeString(String searchStr){
	...
	private static void addSearchListItem(final TreeItem ti, HashSet<String> searchStrSet, boolean highlight) {
    	if (StaticVar.locate.get(ti).type.equalsIgnoreCase("Root") == false)
    		addSearchListItem(ti.getParentItem(), searchStrSet, false);
	...
	//searchFunction
	public static void searchTree(TreeItem ti, HashSet<String> searchStrSet) {
	  ...
	  if (StaticVar.searchchkBox.getValue()){
	   ...
	   if (found){
		...
		for(int i=0; i<ti.getChildCount(); i++){
			searchTree(ti.getChild(i), searchStrSet);
		}
	}

	public static void setState(TreeItem ti, boolean t_f){
		  if (ti.equals(StaticVar.root_ti) == false)
			  setState(ti.getParentItem(), t_f);
			ti.setState(t_f);
	}

	private static void searchMoveNodeTree(TreeItem ti,
			String search_str) {
		// TODO Auto-generated method stub
		if (StaticVar.locate.get(ti).nodeTextBox.getOriginalWidget().getText().toLowerCase().contains(search_str.toLowerCase())){
			((Label) StaticVar.locateMoveNodeMap.get(ti).getWidget()).addStyleName("SearchMatchNode");
			StaticVar.searchListMoveTree.add(StaticVar.locateMoveNodeMap.get(ti));
			setMoveNodeState(ti, true);
	    }else
	    	((Label) StaticVar.locateMoveNodeMap.get(ti).getWidget()).removeStyleName("SearchMatchNode");

		for(int i=0; i<ti.getChildCount(); i++){
			if (StaticVar.locate.get(ti.getChild(i)).isKeyWord())
			  searchMoveNodeTree(ti.getChild(i), search_str);
		}
	}

	public static void setMoveNodeState(TreeItem ti, boolean t_f){
		  if (ti.equals(StaticVar.root_ti) == false)
			  setMoveNodeState(ti.getParentItem(), t_f);
