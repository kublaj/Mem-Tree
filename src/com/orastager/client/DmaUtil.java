...
public class DmaUtil {
	...
	static void appendNode(TreeItem ti){
		if (StaticVar.locateMoveNodeMapRev.get(ti) != null){
	        if (StaticVar.selectedNodePairList.size() == 0){
	          ...
	          SearchUtil.setState(StaticVar.locateMoveNodeMapRev.get(ti), true);
	          Timers.setScrollPosition((StaticVar.locateMoveNodeMapRev.get(ti)).getElement().getOffsetTop());
		    }
		}
	}
	
	static void appendNodePairList(){
    	try{
			for (NodePair nodePair: StaticVar.selectedNodePairList){
				...
				if (nodePair.nodeKeyWord.text.equalsIgnoreCase("")){
					StaticVar.currNode.ti.addItem(nodePair.nodeChildText.ti);
					SearchUtil.setState(StaticVar.currNode.ti, true);
					...
					StaticVar.currNode.ti.addItem(nodePair.nodeKeyWord.ti);
					SearchUtil.setState(StaticVar.currNode.ti, true);
	...
	static void buildMoveNodeTree(final TreeItem root_ti, TreeItem root_moveNode_ti){
		for(int i=0; i<root_ti.getChildCount(); i++){
			if (StaticVar.locate.get(root_ti.getChild(i)).type.equalsIgnoreCase("KeyWord")){
			   if(StaticVar.locateMoveNodeMap.get(root_ti.getChild(i)) == null){
					...
					StaticVar.locateMoveNodeMap.put(root_ti.getChild(i), ti);
					StaticVar.locateMoveNodeMapRev.put(ti, root_ti.getChild(i));
					...
					buildMoveNodeTree(root_ti.getChild(i), ti);
			   }
			   else{
				    ((Label) StaticVar.locateMoveNodeMap.get(root_ti.getChild(i)).getWidget()).setText(StaticVar.locate.get(root_ti.getChild(i)).text);
					buildMoveNodeTree(root_ti.getChild(i), StaticVar.locateMoveNodeMap.get(root_ti.getChild(i)));
			   }
			}
		}
	}
    ...
    static boolean isChildNode(TreeItem ti_paste, boolean cut){
    	...
    	return isChildNode(ti_paste.getParentItem(), cut);
    }
	...
	static void collapseAll(TreeItem ti) {
		for(int i=0; i<ti.getChildCount(); i++){
			collapseAll(ti.getChild(i));
		}
		ti.setState(false);
	}

	static void expandAll(TreeItem ti) {
		for(int i=0; i<ti.getChildCount(); i++){
			expandAll(ti.getChild(i));
		}
		ti.setState(true);
	}
    ...
    static void loadUrls(){
		...
		for (Map.Entry<TreeItem, Node> entry : StaticVar.locate.entrySet()) {
	...
	static void setNodeTextBoxSize(KeyWordLabel label) {
		// TODO Auto-generated method stub		
		if (label.getText().length()>=StaticVar.nodeTextBoxMaxTextWidth)
			label.setWidth(StaticVar.nodeTextBoxMaxTextWidth*StaticVar.charPixelSize+"px");
		else if (label.getText().length()>25 && label.getText().length()<StaticVar.nodeTextBoxMaxTextWidth)
			label.setWidth(label.getText().length()*StaticVar.charPixelSize+"px");
		else
	...
	static int getNodeTextBoxSize(String text) {
		// TODO Auto-generated method stub
		if (text.length()>=StaticVar.nodeTextBoxMaxTextWidth)
			return (int) (StaticVar.nodeTextBoxMaxTextWidth*StaticVar.charPixelSize);
		else if (text.length()>25 && text.length()<StaticVar.nodeTextBoxMaxTextWidth)
			return (int) (text.length()*StaticVar.charPixelSize);
		else
