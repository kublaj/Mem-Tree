...
public class DMA implements EntryPoint{
    ...
    public void onModuleLoad() {
		...
		StaticVar.moveNodeTreeScrPanel.getElement().getStyle().setBackgroundColor("whitesmoke");
		StaticVar.moveNodeTreeScrPanel.getElement().getStyle().setMarginTop(10, Unit.PX);
		StaticVar.moveNodeTreePopPanel.setWidget(StaticVar.moveNodeTreeVPanel);
		StaticVar.moveNodeTreePopPanel.addStyleName("gwt-dmaPopupPanel");
		StaticVar.moveNodeSuggestBox.getTextBox().getElement().setAttribute("placeholder", "Search..");
		StaticVar.moveNodeSuggestBox.setWidth("250px");
		StaticVar.moveNodeTreeVPanel.setTitle("move to..");
		...
		StaticVar.moveNodeSelectButton.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				// TODO Auto-generated method stub
		...
		StaticVar.moveNodeTreeVPanel.add(StaticVar.moveNodeSuggestBox);
		StaticVar.moveNodeTreeVPanel.add(StaticVar.moveNodeTreeScrPanel);
		StaticVar.moveNodeTreeVPanel.add(StaticVar.moveNodeSelectButton);
		...
		StaticVar.previewPopup.addStyleName("gwt-dmaPopupPanel");
		...
		StaticVar.moveNodeSuggestBox.getTextBox().addKeyDownHandler(new KeyDownHandler(){
			public void onKeyDown(KeyDownEvent event) {
				
				  if (StaticVar.moveNodeSuggestBox.getText().equalsIgnoreCase("") == false && event.getNativeKeyCode() == KeyCodes.KEY_ESCAPE){
					  StaticVar.moveNodeSuggestBox.hideSuggestionList();
				  }
				  
				  if (StaticVar.moveNodeSuggestBox.getText().equalsIgnoreCase("") == false && event.getNativeKeyCode() == KeyCodes.KEY_ENTER){
						SearchUtil.searchMoveNodeString(StaticVar.moveNodeSuggestBox.getText());
				  }
				}
			});
		...
		StaticVar.moveNodeSuggestBox.addSelectionHandler(new SelectionHandler<SuggestOracle.Suggestion>(){
			@Override
			public void onSelection(SelectionEvent<Suggestion> event) {
				SearchUtil.searchMoveNodeString(StaticVar.moveNodeSuggestBox.getText());
			}
	    ...
	    StaticVar.jsonPopupPanel.addStyleName("gwt-dmaPopupPanel");
	    ...
	    StaticVar.addNodePairPanel.addStyleName("gwt-dmaPopupPanel");
	    ...
	    StaticVar.addNodePairButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (StaticVar.ctTextArea.getText().equalsIgnoreCase("")){
					...
					if (StaticVar.currNode != null && (StaticVar.currNode.isKeyWord())){
						DmaUtil.appendNodePairList();
					}else DMA.showMsg("Select parent node");
				}
			}
		});
				...
				public void onKeyDown(KeyDownEvent event) {
					...
					if (StaticVar.dma_file == false){
						...
						if (event.isControlKeyDown() && event.getNativeKeyCode() == 70){
							event.preventDefault();
							if (StaticVar.moveNodeTreePopPanel.isShowing() == false) {
								StaticVar.suggestBox.setFocus(true);
								StaticVar.suggestBox.getTextBox().selectAll();
							}else {
								StaticVar.moveNodeSuggestBox.setFocus(true);
								StaticVar.moveNodeSuggestBox.getTextBox().selectAll();
							}
						}
    ...
    public static void loadRootPanel(){
	    ...
	    if (DmaUtil.requestFromMobile()){
	    	...
	    	RootPanel.get("MsgPane").setVisible(false);
	    }
	    else{
